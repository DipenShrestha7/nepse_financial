...
def _extract_quarters(question: str) -> list[str] | None:
    ...
    m2 = re.findall(
        r"\b(\d{3,4})[\/-](\d{2,3})\s*(?:q|Q)?([1-4])\b", question, flags=re.IGNORECASE
    )
    if m2:
        quarters = []
        for a, b, q in m2:
...
def _extract_companies(question: str) -> list[dict]:
    ...
    for company in sorted(
        COMPANIES, key=lambda item: len(item["normalized_name"]), reverse=True
    ):
    ...
    if not matches:
        ...
        text = re.sub(
            r"\b\d{3,4}[-/]\d{2,3}(?:Q[1-4])?\b", "", question
        )  # Remove quarters
        text = re.sub(
            r"\b(?:the|for|of|in|at|on|and|or|is|are|what|where|when|how|why|as|by)\b",
            "",
            text,
            flags=re.IGNORECASE,
        )
        ...
        for token in potential_matches:
            ...
            company_list = [
                {"name": c["company_name"], "symbol": c["company_symbol"]}
                for c in COMPANIES
            ]
...
def _extract_metrics(question: str) -> list[str] | None:
    ...
    if not found:
        ...
        text = re.sub(
            r"\b(?:the|for|of|in|at|on|and|or|is|are|what|where|when|how|why|as|by|a|an)\b",
            "",
            lowered,
            flags=re.IGNORECASE,
        )
...
def parse_question(question: str) -> dict:
    ...
    if companies:
        ...
        if fuzzy_matches and len(fuzzy_matches) == len(companies):
            ...
            parsed["suggestions"]["company_clarification"] = (
                "I found a match, but I'm not 100% sure. Did you mean: "
                + ", ".join(
...
def get_company_suggestions(partial_input: str) -> list[dict]:
    ...
    company_list = [
        {"name": c["company_name"], "symbol": c["company_symbol"]} for c in COMPANIES
    ]
